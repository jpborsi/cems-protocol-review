...
def load_prompt():
    with open("exp002_prompt_oneshot.txt", "r") as f:
        return f.read()


def run(input_filepath, output_filepath, iteration, client, file_id):
    output_file = f"{output_filepath}{iteration}.csv"
    ...
    result = output_data.output_text.replace("```csv", "").replace("```", "")
...
def main(input_filepath, output_file):
    ...
    for i in range(5):
        run(input_filepath, output_file, i, client, file_id)
